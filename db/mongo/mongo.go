...
func (d *DB) Insert(collection *mongo.Collection, ctx context.Context, v any) (primitive.ObjectID, error) {
...
func (d *DB) Update(collection *mongo.Collection, ctx context.Context, id primitive.ObjectID, v any) error {
	res, err := collection.UpdateByID(ctx, id, bson.D{bson.E{Key: "$set", Value: v}})
	if err != nil {
		return err
	}
...
func (d *DB) Upsert(collection *mongo.Collection, ctx context.Context, id primitive.ObjectID, v any) error {
	opts := options.Update().SetUpsert(true)
	_, err := collection.UpdateByID(ctx, id, bson.D{bson.E{Key: "$set", Value: v}}, opts)
	if err != nil {
		return err
	}
	return nil
...
func (d *DB) Delete(collection *mongo.Collection, ctx context.Context, id string) error {
	objID, err := primitive.ObjectIDFromHex(id)
	if err != nil {
		return err
	}
	res, err := collection.UpdateByID(ctx, objID, bson.D{bson.E{Key: "$set", Value: bson.M{
		"is_deleted": true,
	}}})
	if err != nil {
		return err
...
func (d *DB) QueryByID(collection *mongo.Collection, ctx context.Context, id string, result any) error {
	objID, err := primitive.ObjectIDFromHex(id)
	if err != nil {
		return err
	}
	return collection.FindOne(ctx, bson.M{"_id": objID, "is_deleted": false}).Decode(result)
}

func (d *DB) BatchQueryByIDs(collection *mongo.Collection, ctx context.Context, ids []string, result any) error {
	if len(ids) == 0 {
		return nil
	}
...
func (d *DB) PageList(collection *mongo.Collection, ctx context.Context, page uint64, size uint64, filter any, sort map[string]bool, results any) (total int64, err error) {
	if filter == nil {
